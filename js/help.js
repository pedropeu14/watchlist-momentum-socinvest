  ...
  signals: {
    ...
    html: `
      ...
      <table class="gloss">
        ...
        <tr><td>P/E fwd σ</td><td>The Socinvest valuation ruler: the forward P/E (price ÷ projected consensus earnings, weekly Bloomberg series) z-scored against a <strong>rolling 3-year window</strong> — median and 16/84 percentiles, exactly like the Socinvest P/E chart. "Cheap" therefore means cheap vs the current regime, not vs a decade-old panic (a full-history ruler anchored to META's 2022 crash at 9× would never call anything cheap again). <strong>Green ≤ −1σ</strong> = multiple historically cheap; <strong>red ≥ +1σ</strong> = expensive. Crossed with MM200 σ it forms the <em>quadrant analysis</em> in the detail view, shown in two layers: the <strong>position</strong> (sign-based quadrant, same labels as the Socinvest scatter — e.g. "cheap &amp; depressed") and the <strong>verdict</strong> (only named when BOTH rulers pass ±1σ). A stock can sit in the cheap-&amp;-depressed quadrant by a hair's width and still carry no statistical signal — the two layers keep that distinction honest. The pair separates rallies driven by earnings from rallies driven by re-rating. Caveat: the denominator is a <em>projection</em>; estimate revisions move this z without the price moving. Coverage: the assets present in the Bloomberg export (~25); others show "—".</td></tr>
  ...
  strategies: {
    ...
    html: `
      ...
      <table class="gloss">
        ...
        <tr><td>Quadrant Position</td><td>The sign-based map (0-cut, as in the Socinvest scatter): buy while the asset sits in <em>cheap &amp; depressed</em> (both z-scores below zero), sell on reaching <em>expensive &amp; stretched</em> (both above). Trades often; exists mainly as the soft benchmark for Double Depression.</td></tr>
        <tr><td>Double Depression</td><td>The quadrant <em>buy zone</em> at full strength: enter only when BOTH rulers are ≤ −1σ (price depressed and multiple cheap), exit when either stretches to +1σ. The strictest — and rarest — signal in the app; expect few trades, which is the point. Comparing it against Quadrant Position shows what the ±1σ strictness is worth.</td></tr>
